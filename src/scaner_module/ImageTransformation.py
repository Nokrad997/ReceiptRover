...
class TransformImage():
    ...
    def resizeImage(self, base_width=500, base_height=500):
        ...
        dim = (int(self.original.size[0] * self.width_ratio), int(self.original.size[1] * self.height_ratio))
    ...
    def _clickAndMark(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if len(self.manual_contour_points) == 4:
                cv2.line(self.img, self.manual_contour_points[0], self.manual_contour_points[1], (0, 255, 0), 2)
                cv2.line(self.img, self.manual_contour_points[1], self.manual_contour_points[2], (0, 255, 0), 2)
                cv2.line(self.img, self.manual_contour_points[2], self.manual_contour_points[3], (0, 255, 0), 2)
                cv2.line(self.img, self.manual_contour_points[3], self.manual_contour_points[0], (0, 255, 0), 2)
                cv2.imshow("Image", self.img)
                return
    ...
    def findContours(self, debug=False):
        ...
        if self.edged is None:
            self.detect_edges()
        contours, _ = cv2.findContours(self.edged.copy(), cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
        if contours:
    ...
    def correctPerspective(self):
        ...
        if self.contour is None or len(self.contour) != 4:
            raise InterruptedError("Contour not found or invalid for perspective correction")
    ...
    def _contourToRect(self):
        ...
        if len(self.contour) != 4:
            raise ValueError("Contour does not have 4 points and cannot be converted to a rectangle.")
    ...
    def toBlackAndWhite(self, block_size=21, offset=10):
        ...
        if self.scanned is None:
            raise ValueError("Scanned image not available for black and white conversion")
